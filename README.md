...
2. 전화번호 인증<br>
전화번호 인증의 경우는 SMS발송으로 연계해야하지만 서비스를 직접 연결하지 않고 임시로 랜덤으로 생성된 숫자를 응답에 보여주도록 하였습니다.<br>
생성된 숫자는 전화번호를 기준으로 `Redis`에 저장이 되며 3분간의 유효기간을 갖습니다.<br>
해당 숫자로 인증이 완료되면 동일하게 `Redis`에 인증 완료 정보가 저장되고 1일간의 유효기간을 갖습니다.<br>
